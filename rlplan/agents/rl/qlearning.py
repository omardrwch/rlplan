...
class QLearningUcbAgent(Agent):
    """
    Implements Q-learning algorithm for finite MDPs using UCB-like exploration
    ...
    :param learning_rate:
    :param min_learning_rate: minimum learning rate value
    :param c_expl: exploration parameter
    :param rmax: maximum reward value
    :param verbose: if > 0, information is printed
    :param seed_val: integer, seed for np.random.RandomState
    """
    def __init__(self, env, gamma=0.95, learning_rate=None, min_learning_rate=0.05, c_expl=10.0,
                 rmax=1.0, verbose=1, seed_val=42):
        super().__init__()
        # avoid changing the state of original env
        env = deepcopy(env)

        self.id = 'QLearningUcbAgent'
        self.env = env
        self.gamma = gamma
        self.learning_rate = learning_rate
        self.min_learning_rate = min_learning_rate
    ...
    def get_action(self, x):
        q_hat = self.Q[x, :]
        bonus = np.zeros(self.env.action_space.n)
        for action in range(self.env.action_space.n):
            if self.Nsa[x, action] > 0:
                bonus[action] = self.c_expl*np.sqrt(np.log(self.Nsa[x, :].sum())/self.Nsa[x, action])
            else:
                bonus[action] = self.rmax/(1.0-self.gamma)
